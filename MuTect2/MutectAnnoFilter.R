...
splitV <- function(dt){
  ...
  dt[, Depth := sapply(V68, function(x){
    tmp <- unlist(strsplit(x, ":"))
    tmp <- unlist(strsplit(tmp[2], ","))
    return(as.numeric(tmp[1]) + as.numeric(tmp[2]))
  })]
  dt[, MAF := sapply(V68, function(x){
    tmp <- unlist(strsplit(x, ":"))
    return(as.numeric(tmp[3]))
  })]
...
if (length(args)==0) {
  ...
  # default output file
  args[2] = basename(args[1]) %>% str_replace(replacement = "", pattern = "\.vcf.*.txt") %>% paste0(".filteredAnnotation.txt")
}
dn <- dirname(args[1])

## read in files
dt <- fread(args[1], sep="\t") %>% splitV %>% dbFilter
dt <- dt[V65 == "PASS" & Func.refGene == "exonic" & !is.element(ExonicFunc.refGene, c("synonymous SNV"))]
